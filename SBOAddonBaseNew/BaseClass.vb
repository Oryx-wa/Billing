...
Public MustInherit Class UserFormBaseClass
    ...
    Protected m_parentForm As UserFormBaseClass
    ...
    Protected Friend Overridable Sub InitBase(ByVal pAddOn As SboAddon)
    ...
    Protected Overridable Sub CreateObject(ByVal pObjRef As SBOBaseObject)
    ...
    Protected Function HandleChooseFromListEvent2(FormUID As String, ByRef pVal As SAPbouiCOM.SBOItemEventArg, ByRef BubbleEvent As Boolean) As String
        Dim iSBOChooseFromListEventArg As ISBOChooseFromListEventArg = CType(pVal, ISBOChooseFromListEventArg)
        Dim result As String = ""
        Dim selectedObjects As DataTable = iSBOChooseFromListEventArg.SelectedObjects
        If selectedObjects IsNot Nothing Then
            Try
                result = selectedObjects.GetValue(0, 0).ToString()
            Catch ex As Exception
                Me.m_ParentAddon.SboApplication.SetStatusBarMessage(ex.Message, BoMessageTime.bmt_Short, True)
            End Try
        End If
        Return result
    End Function

    Protected Function HandleChooseFromListEvent2(FormUID As String, ByRef pVal As SAPbouiCOM.SBOItemEventArg, ByRef BubbleEvent As Boolean, ByRef lData As Boolean) As DataTable
        Dim iSBOChooseFromListEventArg As ISBOChooseFromListEventArg = CType(pVal, ISBOChooseFromListEventArg)
        Dim selectedObjects As DataTable = iSBOChooseFromListEventArg.SelectedObjects
        If selectedObjects IsNot Nothing Then
            Try
    ...
    Public Overridable Sub parentFormRefresh(strID As String)
    End Sub
    ...
    Private Sub UserFormBaseClass_CloseAfter(ByVal pVal As SAPbouiCOM.SBOItemEventArg) Handles Me.CloseAfter
        If Not IsNothing(m_BaseObject) Then
            m_BaseObject.Dispose()
            GC.Collect()
        End If
    End Sub

    Private Sub UserFormBaseClass_UnloadAfter(ByVal pVal As SAPbouiCOM.SBOItemEventArg) Handles Me.UnloadAfter
        If Not IsNothing(m_BaseObject) Then
            m_BaseObject.Dispose()
            GC.Collect()
        End If
